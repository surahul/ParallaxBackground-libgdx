...
public class ParallaxBackground {
	...
	public void draw(OrthographicCamera worldCamera, Batch batch){
		...
		for(int i=0; i<layers.size; i++){
		    ...
		    float currentX = (layer.getTileModeX().equals(TileMode.single)?0:((int)((worldCamera.position.x-worldCamera.viewportWidth*.5f) / layer.getWidth())) * layer.getWidth())-(Math.abs((1-layer.getParallaxRatio().x)%1)*worldCamera.viewportWidth*.5f);
			do{
		            float currentY =  (layer.getTileModeY().equals(TileMode.single)?0:((int)((worldCamera.position.y-worldCamera.viewportHeight*.5f) / layer.getHeight())) * layer.getHeight())-(((1-layer.getParallaxRatio().y)%1)*worldCamera.viewportHeight*.5f);
		            do{
		               if(!((worldCamera.position.x-worldCamera.viewportWidth*.5f>currentX+layer.getWidth())||(worldCamera.position.x+worldCamera.viewportWidth*.5f<currentX)||(worldCamera.position.y-worldCamera.viewportHeight*.5f>currentY+layer.getHeight())||(worldCamera.position.y+worldCamera.viewportHeight*.5f<currentY)))
		                   layer.draw(batch, currentX, currentY);
		               currentY += layer.getHeight();
		               if(layer.getTileModeY().equals(TileMode.single))
			        	     break;
		            }while( currentY < worldCamera.position.y+worldCamera.viewportHeight*.5f);
		            currentX += layer.getWidth();
		            if(layer.getTileModeX().equals(TileMode.single))
		        	     break;
		         }while( currentX < worldCamera.position.x+worldCamera.viewportWidth*.5f);
